...
namespace Moira.ApiClient.Model
{
    ...
    public class DtoTriggerJsonConverter : JsonConverter<DtoTrigger>
    {
        ...
        public override DtoTrigger Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            return new DtoTrigger(aloneMetrics, clusterId, createdAt, createdBy, desc, errorValue, expression, id, isRemote, muteNewMetrics, name, patterns, sched, tags, targets, throttling, triggerSource, triggerType, ttl, ttlState, updatedAt, updatedBy, warnValue);
        }
        ...
        public void WriteProperties(Utf8JsonWriter writer, DtoTrigger dtoTrigger, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (dtoTrigger.ErrorValueOption.IsSet)
                if (dtoTrigger.ErrorValueOption.Value != null)
                    writer.WriteNumber("error_value", dtoTrigger.ErrorValueOption.Value.Value);
                else
                    writer.WriteNull("error_value");

            if (dtoTrigger.ExpressionOption.IsSet)
                writer.WriteString("expression", dtoTrigger.Expression);

            if (dtoTrigger.IdOption.IsSet)
                writer.WriteString("id", dtoTrigger.Id);

            if (dtoTrigger.IsRemoteOption.IsSet)
                writer.WriteBoolean("is_remote", dtoTrigger.IsRemoteOption.Value.Value);

            if (dtoTrigger.MuteNewMetricsOption.IsSet)
                writer.WriteBoolean("mute_new_metrics", dtoTrigger.MuteNewMetricsOption.Value.Value);

            if (dtoTrigger.NameOption.IsSet)
                writer.WriteString("name", dtoTrigger.Name);

            if (dtoTrigger.PatternsOption.IsSet)
            {
            ...
            if (dtoTrigger.TagsOption.IsSet)
            {
                writer.WritePropertyName("tags");
                JsonSerializer.Serialize(writer, dtoTrigger.Tags, jsonSerializerOptions);
            }
            if (dtoTrigger.TargetsOption.IsSet)
            {
                writer.WritePropertyName("targets");
                JsonSerializer.Serialize(writer, dtoTrigger.Targets, jsonSerializerOptions);
            }
            if (dtoTrigger.ThrottlingOption.IsSet)
                writer.WriteNumber("throttling", dtoTrigger.ThrottlingOption.Value.Value);

            if (dtoTrigger.TriggerSourceOption.IsSet)
                writer.WriteString("trigger_source", dtoTrigger.TriggerSource);

            if (dtoTrigger.TriggerTypeOption.IsSet)
            ...
            if (dtoTrigger.UpdatedAtOption.IsSet)
                if (dtoTrigger.UpdatedAtOption.Value != null)
                    writer.WriteString("updated_at", dtoTrigger.UpdatedAt);
                else
                    writer.WriteNull("updated_at");

            if (dtoTrigger.UpdatedByOption.IsSet)
                writer.WriteString("updated_by", dtoTrigger.UpdatedBy);

            if (dtoTrigger.WarnValueOption.IsSet)
                if (dtoTrigger.WarnValueOption.Value != null)
                    writer.WriteNumber("warn_value", dtoTrigger.WarnValueOption.Value.Value);
                else
